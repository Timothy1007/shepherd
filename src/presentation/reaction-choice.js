...
const QA_DISASTER_ID='disaster-01';
...
function moveDefinitionToPlayer(state,definitionId,target){
  if(target.hand.some(c=>c.definitionId===definitionId))return;
  let incoming=null,source=null;
  const deckIndex=state.deck.findIndex(c=>c.definitionId===definitionId);
  if(deckIndex>=0){incoming=state.deck.splice(deckIndex,1)[0];source=state.deck;}
  if(!incoming){for(const player of state.players.filter(p=>p!==target)){const index=player.hand.findIndex(c=>c.definitionId===definitionId);if(index>=0){incoming=player.hand.splice(index,1)[0];source=player.hand;break;}}}
  if(!incoming)return;
  const replaceIndex=target.hand.findIndex(c=>c.definitionId!==REACTION_ID&&c.definitionId!==QA_DISASTER_ID);
  const replaced=replaceIndex>=0?target.hand.splice(replaceIndex,1,incoming)[0]:target.hand.pop();
  if(replaceIndex<0)target.hand.push(incoming);
  if(replaced)source.push(replaced);
}
function guaranteePreviewSetup(controller){
  if(controller.state?.qaSeed!=='recovery-preview'||controller.state.round!==1)return;
  const state=controller.state,human=humanOf(controller),ai=state.players.find(p=>p.type==='ai');
  moveDefinitionToPlayer(state,REACTION_ID,human);
  if(ai)moveDefinitionToPlayer(state,QA_DISASTER_ID,ai);
}
...
GameController.prototype.runAiTurn=function(generation,playerId){if(pending?.controller===this){this.cancelTimer();return;}if(generation!==this.generation||!this.state||this.state.gameOver)return;const actor=this.state.players.find(p=>p.playerId===this.state.currentPlayer);if(actor?.type==='ai'&&actor.playerId===playerId){const actions=getNormalActions(this.state);const disasterAction=actions.find(a=>isDisasterAction(this,a));const action=disasterAction??actions[0];if(action&&isDisasterAction(this,action)){const info=disasterInfo(this,action),human=humanOf(this);if(reactionCard(human)&&affectsPlayer(action,info.definition.definitionId,human.playerId)){show(this,action,this.snapshot(),generation,playerId);return;}}}
