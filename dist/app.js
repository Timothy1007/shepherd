...
cards.forEach((c,i)=>{const b=document.createElement('button');b.type='button';b.className='hand-card';b.dataset.id=c.id;b.style.setProperty('--tilt',(i-3.5)*1.4+'deg');b.style.setProperty('--arc',Math.abs(i-3.5)*2+'px');b.setAttribute('aria-label','查看'+c.type+'：'+c.name+(c.number?c.number:''));b.append(originalCard(c));b.addEventListener('click',()=>{selectCard(c.id);dialog.showModal();});b.addEventListener('keydown',e=>{if(e.key==='ArrowRight'||e.key==='ArrowLeft'){e.preventDefault();const j=(i+(e.key==='ArrowRight'?1:-1)+cards.length)%cards.length;hand.children[j].focus();selectCard(cards[j].id)}});hand.append(b)});selectCard(selected);document.querySelectorAll('.hand-card').forEach(b=>b.setAttribute('aria-pressed','false'));document.getElementById('selection-status').textContent='點選一張卡牌';
