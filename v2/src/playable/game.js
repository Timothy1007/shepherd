...
export function pass(state,pid){if(state.currentPlayer!==pid)throw new Error('Not this player turn');state.passStreak++;log(state,`${player(state,pid).name} 跳過行動。`,'pass');if(state.phase==='death-round'){moveToNext(state);return state;}if(state.passStreak>=state.players.length){settleRound(state);return state;}moveToNext(state);return state;}
